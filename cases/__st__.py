...
class MySignalHandler:
    TEST_RET_COL_NO = 9  # 测试结果在用例excel文件中的列数
    path = r'D:\tools\projects\auto_yjyj\log\测试用例_yjyx.xlsx'
    ...
    def case_result(self, case):
        ...
        # 找到对应的测试用例在excel中的行数
        print(case.name)
        # if ('学生登录' in case.name or '老师登录' in case.name) and len(case.name.split('_')) == 2:
        #     # case.name 学生登录-0205
        #     idd = case.name.split('_')[1][1:]
        #     caseNo = f'TC_LOGINLOGOUT_{idd}'
        # else:
        caseNo = '_'.join(case.name.split('_')[1:])
        print(caseNo)
    ...
    def test_end(self, runner):
        ...
        for case in runner.case_list:
            if hasattr(case, 'execRet'):
                print(f'{case.name} --- {case.execRet}')
            else:
                print(f'{case.name} --- 未执行或初始化失败')
